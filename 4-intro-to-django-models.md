...
```python
>>> c = Cat(name="Penelope", breed='Tortie',
>>> description='Cute as a Disney cat. Looks pink.', age=4)
>>> c.__dict__
{..., 'id': None, 'name': 'Penelope', 'breed': 'Tortie',
 'description': 'Cute as a Disney cat. Looks pink.', 'age': 4}
```
...
For example, this query would return all cats with the name "Penny":

```python
>>> Cat.objects.filter(name='Penny')
<QuerySet [<Cat: Penny>]>
```
...
```python
>>> Cat.objects.filter(name__contains='Pen')
<QuerySet [<Cat: Penny>]>
```
The above's SQL equivalent would be something like:

```sql
SELECT * FROM main_app_cat WHERE name LIKE '%Pen%';
```

As another example, here's how we can find cats that have an age _equal to or less than_ 5:

```python
>>> Cat.objects.filter(age__lte=5)
<QuerySet [<Cat: Penny>]>
```
...
```sql
SELECT * FROM main_app_cat WHERE age <= 5;
```
...
<img src="https://i.imgur.com/BBjzOQt.png">
...
<img src="https://i.imgur.com/aVuiEYD.png">
