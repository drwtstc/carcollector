...
```python
>>> Cat.objects.all()
<QuerySet [<Cat: Penny>, <Cat: Olivia>, <Cat: Logan>, <Cat: Walter>]>
```
...
```python
>>> cat = Cat.objects.get(name='Penny')
```
...
```python
>>> cat.toys.all()
<QuerySet []>   # Penny has no toys associated with her yet
```
...
<QuerySet [<Toy: Bouncy Mouse>, <Toy: Wacky Mouse Chaser>]>
```
...
```python
>>> first_toy.cat_set.all()
<QuerySet [<Cat: Penny>, <Cat: Walter>]>   # Associated with two cats
>>> cat    # Going to unassociate this cat
<Cat: Penny>
>>> first_toy.cat_set.remove(cat)
>>> first_toy.cat_set.all()
<QuerySet [<Cat: Walter>]>    # No more Penny
```
...
<img src="https://i.imgur.com/h9jHMwK.png">
...
<img src="https://i.imgur.com/lHB1ZWj.png">
...
If you browse to **Add a Cat**, you'll notice that the form has an input for the new cat's "toys".  This extra input will prevent us from being able to add a cat, so let's refactor the `fields` attribute of the `CatCreate` CBV as follows:
